...
def poll_replies():
    ...
    try:
        with MailBox(IMAP_HOST).login(IMAP_USER, IMAP_PASS) as mailbox:
            ...
            pending_outreach = Outreach.objects.filter(
            ...
            for outreach in pending_outreach:
                ...
                messages = mailbox.fetch(
                    AND(from_=contact_email), limit=5, reverse=True
                )

                for msg in messages:
                    # simplistic check: if they sent us an email after we sent ours, count it as a reply
                    if outreach.sent_at and msg.date.replace(
                        tzinfo=None
                    ) > outreach.sent_at.replace(tzinfo=None):
